        ...
        NavigationStack {
            ZStack {
                ...
                if store.items.isEmpty {
                    ContentUnavailableView("No Detours Yet", systemImage: "tray", description: Text("Tap + to log your first one."))
                } else {
                    List {
            ...
            .navigationTitle("Roadwork Detour")
            .toolbar {
...
struct AddItemView: View {
    ...
    var body: some View {
        NavigationStack {
            Form {
                Section {
                    TextField("Route", text: $titleText)
                        .focused($focusedField, equals: .title)
                        .accessibilityIdentifier("addTitleField")
                    TextField("Reason", text: $value2Text)
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Save") {
                        let item = Detour(title: titleText.isEmpty ? "Detour" : titleText, value2: value2Text, notes: notesText)
                        if store.add(item) {
                            dismiss()
                        }
                    }
